...
const string keys =
    ...
    "{@Kernel kernel k K |  3  | Indicates the Kernel size (ODD values only)}"
...
int main (int argc, char* const* argv)
{
    ...
    try
    {
        ...
        // Transform the original image to 3 channels if needed
        if (Original.channels() == 1)
        {
            cv::cvtColor(Original, Unsharp, cv::COLOR_GRAY2RGB);
        }
        // Otherwise copy the original image to the output
        else
        {
